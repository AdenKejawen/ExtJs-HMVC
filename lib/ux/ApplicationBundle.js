Ext.define('Ext.app.ApplicationBundle', {
    ...
    constructor: function(config) {
        ...
        this.appFolder = this.appFolder||(this.id && this.id.toLowerCase())||'app';
        
        if(this.appFolder && this.application){
            this.appFolder = this.application.appFolder+'/bundles/'+this.appFolder;
        }
        ...
        Ext.Loader.setPath(this.name||this.id, this.appFolder);
        ...
        if (this.autoCreateViewport && !this.application) {
        ...
        for (i = 0; i < bundles.length; i++) {
            requires.push((this.name||this.id) + '.bundles.' + bundles[i]);
        }
    ...
    onBeforeLaunch: function() {
        ...
        var viewInjection = function(name, container, scope) {
            var view = scope.getView(name), instance;
            scope.fireEvent('shellBeforeLaunch', scope);
            if (!view){
                console.log('WARNING: Bundle ' + scope.name
                    + ' not define ' + name + ' view');
            }else{
                instance = view.create();
                container.add(instance);
                container.doLayout();
            }
            scope.fireEvent('shellLaunch', instance, scope);
        }
        ...
        if (this.autoCreateViewport && !this.application) {
            ...
            if(this.placeholder){
                var nameView = this.placeholder.shell || 'Main';
                // view discovery
                var selecteds = Ext.ComponentQuery.query(this.placeholder.selector);
                viewInjection(nameView, selecteds[0], this);
            }
        }
    ...
    getBundle: function(name) {
        ...
        if (!bundle) {
            ...
            //Ext.ClassManager.setNamespace(this.name+'.bundles.'+name, bundle);
